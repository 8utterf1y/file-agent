...
`exact` 模式结果不能代表“没有近重复”。`standard` 是默认平衡模式。`deep` 模式结果更完整，但成本更高。
...
脚本不会真正完成导入、引用和调用方分析。`call_reference_analysis.status=not_checked_by_script` 时，Agent 必须在最终建议中说明检查状态。
