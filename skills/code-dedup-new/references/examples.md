...
```text
当前使用 standard 模式，包含完全重复和常规近重复分析。
```
...
```text
当前使用 deep 模式，成本较高，会展开更多近重复候选。如果达到比较上限，结论仍可能不完整。
```
...
1. 运行 `scripts/code_dedup.py --mode standard` 生成 JSON/Markdown。
...
源码复核：两处输入输出基本一致，但还需确认调用方和异常处理。
建议：试运行建议抽到 src/shared/normalize_record.py；requires_manual_review=true。
```
...
```text
只检查 src 和 packages，跳过 tests，只给高/中优先级的重复和合并建议。
```
...
调用关系检查：脚本未完成；需要继续搜索导入、引用和调用方。
建议：不要直接合并整个函数，只能人工确认后抽取最小公共辅助逻辑。
```
